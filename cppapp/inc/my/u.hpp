...
#define DBG_LOG(msg)                                                           \
  do {                                                                         \
    char __tmstr[25]{'\0'};                                                    \
    std::cout << tmstr(__tmstr) << " : (" << __FILE__ << ":" << __LINE__       \
              << "), " << msg << '\n';                                         \
  } while (false)
#else
#define DEB_LOG(msg)                                                           \
...
inline const char *tmstr(char *tstr) {
  ...
  sprintf_s(tstr, 25, "%4u:%02u:%02u %02u:%02u:%02u:%03u", st.tm_year + 1900,
            st.tm_mon, st.tm_mday, st.tm_hour, st.tm_min, st.tm_sec,
            (unsigned int)(ts.tv_nsec / 1e6));
  return tstr;
#endif
}

static const char *__err_arg0 = "unknown";
inline void merr_setargs0(const char *arg0) { __err_arg0 = arg0; }
